...
input_field = Entry(input_frame, font =('arial', 18, 'bold'), width=35, justify=RIGHT, textvariable=input_text)
...
# Button Frame
btns_frame = Frame(win, width=312, height=270)
btns_frame.pack()

clear = Button(btns_frame, text='C', width=35, height=3, command=lambda:bt_clear()).grid(row=0, column=0, columnspan = 3, padx=1, pady=1)
division = Button(btns_frame, text='/', width=10, height=3, command=lambda:btn_click('/')).grid(row=0, column=3, padx=1, pady=1)
